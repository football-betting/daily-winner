...
class DataProviderGenerator implements DataProviderGeneratorInterface
{
    ...
    public function generate(): array
    {
        ...
        foreach ($this->parser->getDataProvider() as $providerName => $providerData) {
            ...
            foreach ($providerData['elements'] as &$property) {

                if (strpos($property['type'], '\App\DataTransferObject\\') !== false) {
                    $component = explode('\\', $property['type']);
                    $component = $component[count($component) - 1];
                    $component = str_replace('DataProvider', '', $component);
                    $component = str_replace('[]', '', $component);

                    $componentDir = __DIR__ . '/../../../../../Component/' . $component;

                    if (is_dir($componentDir)) {
                        $property['type'] = str_replace(
                            'DataTransferObject',
                            'Component\\' . $component . '\Domain\DataTransferObject',
                            $property['type']
                        );

                        if (strpos($property['singleton'], '\App\DataTransferObject\\') !== false) {
                            $property['singleton'] = str_replace(
                                'DataTransferObject',
                                'Component\\' . $component . '\Domain\DataTransferObject',
                                $property['singleton']
                            );
                        ...
                        if (strpos($property['singleton_type'], '\App\DataTransferObject\\') !== false) {
                            $property['singleton_type'] = str_replace(
                                'DataTransferObject',
                                'Component\\' . $component . '\Domain\DataTransferObject',
                                $property['singleton_type']
                            );
                        }
                    } else {
                        $namespace->addUse(
                            substr(str_replace('[]', '', $property['type']), 1)
                        );
                    }
                }
            }
            ...
            $useString = '';
            foreach ($namespace->getUses() as $dataProviderName => $use) {
                if ($dataProviderName !== $providerName . 'DataProvider') {
                    $useString .= 'use ' . $use . ';' . "\n";
                }
            }

            if ($useString) {
                $classContent = str_replace('namespace App\DataTransferObject;', 'namespace App\DataTransferObject;' . "\n\n" . $useString, $classContent);
            }
